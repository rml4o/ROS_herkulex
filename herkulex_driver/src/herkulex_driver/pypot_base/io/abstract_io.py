...
class AbstractHkxIO(AbstractIO):
    ...
    def _get_control_value(self, control, ids, **kwargs):
        ...
        return_status_bytes = kwargs['return_status_bytes'] if ('return_status_bytes' in kwargs) else False
        ...
        for motor_id in ids:
            ...
            status_bytes.append(tuple([sp.error, sp.status_detail]))
        ...
        if return_status_bytes:
            status_bytes = tuple(status_bytes)
            zop = zip(values, status_bytes)
            return [a + b for a, b in zop]
        else:
            return tuple(values)
